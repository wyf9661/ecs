...
func ecsSaveCreateLayer(bundlePath string, workDir string) (string, error) {
	...
	log.Println("Creating layerJson...")
...
func ecsSaveImageConfig(workDir string, layerSha256 string) (string, error) {
	...
	parser.GlobalImageConfig.Config.Labels[`sylixos.resoarDirectoryurces.kernelObject.srtpLimit`] = strconv.Itoa(parser.GlobalConfigStruct.Sylixos.Resources.KernelObject.SrtpLimit)
...
func ecsSaveCreateRepositories(workDir string, layerSha256 string, imageName string, imageTag string) error {
	result := fmt.Sprintf("{\n\t\"%s\":{\n\t\t\"%s\":\"%s\"\n\t}\n}", strings.TrimSuffix(imageName, ".tar"), imageTag, layerSha256)
...
func ecsSaveCreateManifest(workDir string, layerSha256 string, configSha256 string, imageName string, imageTag string) error {
	...
	parser.GlobalImageInfos[0].RepoTags[0] = strings.TrimSuffix(imageName, ".tar") + ":" + imageTag
...
func EcsSave(bundlePath string, imageName string, imageVersion string) error {
	...
	configSha256, err := ecsSaveImageConfig("./ecs_temp", layerSha256)
	if err != nil {
		log.Println("Failed to create layer!")
	}

	if err := ecsSaveCreateRepositories("./ecs_temp", layerSha256, imageName, imageVersion); err != nil {
		log.Println("Failed to create repositories!")
	}

	if err := ecsSaveCreateManifest("./ecs_temp", layerSha256, configSha256, imageName, imageVersion); err != nil {
		log.Println("Failed to create repositories!")
	}
